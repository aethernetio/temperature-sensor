 ...
 *
 * Silent 5x20 prepared KEEP-WIFI-UP experiment (ESP32-C6):
 * 1 registration, 5 FULL cycles each preparing 20 messages.
 * Wi-Fi associates once per outer cycle; 20 UDP sends reuse the association.
 * Per-message timing is encode+sendto only (no post-send hold).
...
#  define BENCH_CLIENT_ID "prepared_keep_wifi_up_5x20_v1"
#endif
...
static ae::DataBuffer MakePreparedPayload(int outer, int index) {
  ...
  p.sequence_global = NextSeq();
  // For index==1: previous_full_us carries wifi session start µs.
  p.previous_full_us = (index == 1) ? g_last_session_start_us : 0;
  p.previous_prepared_us = (index == 1) ? 0 : g_last_prepared_us;
  // cache_flags describe the Wi-Fi session (set at Begin).
  p.cache_flags = g_sticky_cache_flags;
  return bench::Encode<ae::DataBuffer>(p);
}
...
static void StartPreparedPhase() {
  ...
  g_wifi_session_open = false;
  g_last_session_start_us = 0;
#if defined(ESP_PLATFORM)
  prepared_send::ReleaseFullAetherWifiForHotPath();
  if (!prepared_send::BeginPreparedWifiSession()) {
    // Session failed — leave session closed; sends will report wifi-failed.
    g_wifi_session_open = false;
  } else {
    g_wifi_session_open = true;
    g_last_session_start_us = prepared_send::LastWifiSessionStartUs();
...
#if defined(ESP_PLATFORM)
      if (g_wifi_session_open) {
        prepared_send::EndPreparedWifiSession();
        g_wifi_session_open = false;
      }
#endif
...
#if defined(ESP_PLATFORM)
    auto const status =
        prepared_send::SendPreparedPacketOnActiveWifi(payload);
#else
    auto const status = prepared_send::HotSendStatus::kUnsupported;
#endif
