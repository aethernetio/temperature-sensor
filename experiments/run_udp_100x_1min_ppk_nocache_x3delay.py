#!/usr/bin/env python3
"""UDP 100x / 1 min / PPK: no Wi-Fi RTC cache + 3x settle/hold delays.

Same measurement method as the cached baseline, but firmware is built with:
  AE_UDP_WIFI_CACHE=0
  AE_UDP_PRE_SETTLE_MS=150   (3x of 50)
  AE_UDP_POST_SEND_HOLD_MS=600  (3x of 200)

Cold-boot 60 s flash window unchanged. Retry/TX/payload/teardown/GPIO unchanged.
...
BUILD = ROOT / "build-udp-100x-1min-ppk-nocache-x3"
RESULTS = HERE / "power_factor_results"
ART_DIR = RESULTS / "udp_100x_1min_ppk_nocache_x3_windows"
CSV_OUT = RESULTS / "udp_100x_1min_ppk_nocache_x3_sends.csv"
JSON_OUT = RESULTS / "udp_100x_1min_ppk_nocache_x3.json"
MD_OUT = HERE / "UDP_100X_1MIN_PPK_NOCACHE_X3DELAY.md"

UDP_HOST = "192.168.68.84"
UDP_PORT = 9000
PRE_SETTLE_MS = 150  # 3x baseline 50
POST_HOLD_MS = 600  # 3x baseline 200
WIFI_CACHE = 0
MEASURED = 100
PERIOD_S = 60.0
VOLTAGE_MV = 3000
...
def build_firmware() -> None:
    ...
    defs = (
        ...
        f"-D AE_UDP_WIFI_CACHE={WIFI_CACHE} "
        f"-B '{b}'"
    )
    extra = f"""
...
class SendBurstStreamer:
    ...
    def _segment_one(self, idx: int, t_udp: float) -> dict | None:
        ...
        elif charge_mC < 0.5 or charge_mC > 600.0:
            valid = False
...
def write_md(rep: dict) -> None:
    ...
    lines = [
        ...
        f"- wifi_cache: {WIFI_CACHE} (channel/BSSID/static IP/ARP all off)",
        f"- pre_settle_ms: {PRE_SETTLE_MS} (3x baseline 50)",
        f"- post_send_hold_ms: {POST_HOLD_MS} (3x baseline 200)",
