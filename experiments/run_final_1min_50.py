#!/usr/bin/env python3
"""Final 1-minute cadence energy test: 1 FULL + 50 HOT per AP.

Fixed encode-during-association (late socket bind). One PPK integral covers
FULL prepare through HOT #50 final deep sleep. No full-rate CSV.
"""
...
BUILD_ROOT = ROOT / "build-final-1min-50" / RUN_ID
RESULTS_DIR = ROOT / "experiments" / "power_factor_results"
RAW_DIR = ROOT / "experiments" / "power_modes_raw" / "final_1min_50"
REPORT_MD = ROOT / "experiments" / "PREPARED_FINAL_1MIN_50_REPORT.md"
RESULT_JSON = RESULTS_DIR / "final_1min_50.json"
PROGRESS = ROOT / "experiments" / "final_1min_50_progress.log"
...
# FULL + 50*60s + settle margin.
RUN_TIMEOUT_S = 4500
IDLE_AFTER_LAST_SEQ_S = 90
POLL_S = 2.0

APS_ORDER = ["chirkov", "aethernetio"]


def apply_run_id(run_id: str) -> None:
    global RUN_ID, BUILD_ROOT
    RUN_ID = run_id
    BUILD_ROOT = ROOT / "build-final-1min-50" / RUN_ID


def log(msg: str) -> None:
    line = time.strftime("%H:%M:%S") + " FINAL50 " + msg
    try:
...
def cmake_configure(ap: str) -> None:
    ...
    extra = {
        ...
        # FINAL_1MIN selects silent sdkconfig + Done-after-HOT path.
        "AE_EXP_PREPARED_FINAL_1MIN_100": "1",
        "AE_EXP_ENCODE_DURING_ASSOCIATION": "1",
        # Fixed late-socket path — never the legacy early bind.
...
def build_firmware(ap: str) -> None:
    ...
    pf.camp.BUILD = bdir
    want = (
        f"{ap}:final50:v{VARIANT_ID}:r{K_HOT_ATTEMPTS + 10}:"
        f"enc1:late_socket:early1"
    )
    have = (
        pf.CONFIGURED_MARK.read_text(encoding="utf-8").strip()
...
def parse_rx_log(text: str) -> dict:
    ...
    seqs = [
        int(m.group(1)) for m in re.finditer(r"BENCH_DATA .* seq=(\d+)", text)
    ]
    unique = len(set(seqs))
    dup = len(seqs) - unique
...
def wait_run_done(rx_log: Path, ppk_proc: subprocess.Popen) -> dict:
    ...
    arm_at = None
    last_seq_at = None
    # Cadence after BENCH_ARM (~50*60s + settle), not wait for RX=50.
    ...
    while time.time() - t0 < RUN_TIMEOUT_S:
        ...
        if rx["max_seq"] >= K_HOT_ATTEMPTS:
            if last_seq_at is None:
                last_seq_at = time.time()
                log(f"seq{K_HOT_ATTEMPTS} seen unique={rx['rx_unique']}")
            elif time.time() - last_seq_at >= IDLE_AFTER_LAST_SEQ_S:
                log("idle after last seq — run complete")
                return rx
        if arm_at is not None and (time.time() - arm_at) >= cadence_s:
            ...
            return rx
        if int(time.time() - t0) % 60 < POLL_S:
            log(
                f"progress unique={rx['rx_unique']}/{K_HOT_ATTEMPTS} "
                f"max_seq={rx['max_seq']} elapsed={time.time() - t0:.0f}s"
            )
        time.sleep(POLL_S)
...
def run_ap(ap: str) -> dict:
    ...
    rx_log = ROOT / "experiments" / "final_1min_50_rx.log"

    build_firmware(ap)
    # Fresh RX log per AP so prior HOT seqs cannot satisfy the waiter.
    ...
    row = {
        ...
        "run_id": RUN_ID,
        "pre_ms": 0 if ap == "aethernetio" else 25,
    }
    log(
...
def write_report(rows: list[dict]) -> None:
    lines = [
        ...
        "",
        "Config: SKIP_VALIDATE=yes CPU=80 FULL_TEARDOWN=yes DISC_PM=ON "
        "WIFI_PS=NONE ENCODE_DURING_ASSOCIATION=yes (late socket) "
        "PRE chirkov=25 ms / aethernetio=0 ms POST=0 TX-done TaskNotify.",
        "",
...
def main() -> int:
    ...
    ap.add_argument(
        ...
        type=Path,
        default=None,
        help="Existing final_1min_50.json to merge prior AP rows",
    )
    ap.add_argument(
        "--run-id",
        default=None,
        help="Reuse an existing build-final-1min-50/<run-id> directory",
    )
    args = ap.parse_args()
    if args.run_id:
