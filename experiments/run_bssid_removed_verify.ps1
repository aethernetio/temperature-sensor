...
$log = Join-Path $exp 'v40_bssid_removed_capture.log'
$rebootLog = Join-Path $exp 'v40_bssid_removed_reboot3.log'
...
# Production baseline driver after BSSID removal. Do NOT erase SPIFFS.
$cmakeArgs = @(
  '-B', $build,
  '-D', 'WIFI_SSID=chirkov',
  ...
  '-D', 'AE_EXP_WIFI_LIFECYCLE_VARIANT=40',
  ...
  '-D', 'AE_EXP_WIFI_IGNORE_BSSID=',
  '-D', 'AE_EXP_WIFI_FEAT_BSSID_CACHE='
)

Write-Output 'BUILD variant=40 production BSSID-removed (no flash erase)'
